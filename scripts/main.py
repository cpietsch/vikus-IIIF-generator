...
def list_instances():
    instances = []
    paths = sorted(
        filter(os.path.isdir, Path(DATA_DIR).iterdir()),
...
def read_instance(instance_id: str):
    ...
    if not os.path.isfile(configFile):
        return {
            "error": "Instance {} doesn't have a config file".format(instance_id)
        }, 404
...
async def crawl_collection(
    ...
    worker: int = Query(
        DEFAULTS["collection"]["worker"],
        title="Workers",
        description="Number of workers",
    ),
    depth: int = Query(
        DEFAULTS["collection"]["depth"],
        title="Depth",
        description="Recursive crawl depth",
        min=0,
        max=100,
    ),
    skip_cache: bool = Query(
        DEFAULTS["collection"]["skip_cache"],
        title="Skip cache",
    ...
    manifests = await crawlCollection(
        config["iiif_url"], instance_id, worker, depth, skip_cache
    )
    ...
    InstanceManager[instance_id].update({"manifests": manifests})
...
async def crawl_images(
    instance_id: str = Query(title="Instance ID", description="Instance ID"),
    worker: int = Query(
        DEFAULTS["images"]["worker"], title="Workers", description="Number of workers"
    ),
    skip_cache: bool = Query(
    ...
    """
    if (
        instance_id not in InstanceManager
        or "manifests" not in InstanceManager[instance_id]
    ):
        await crawl_collection(
            instance_id,
            worker=DEFAULTS["collection"]["worker"],
            depth=DEFAULTS["collection"]["depth"],
            skip_cache=DEFAULTS["collection"]["skip_cache"],
        )
    ...
    InstanceManager[instance_id].update({"images": images})
...
async def make_metadata(
    ...
    metadata = await makeMetadata(
        manifests, instance_id, config["path"], skip_cache=skip_cache
    )
...
async def make_spritesheets(
    ...
    if (
        instance_id not in InstanceManager
        or "images" not in InstanceManager[instance_id]
    ):
        await crawl_images(
            instance_id,
            worker=DEFAULTS["images"]["worker"],
            skip_cache=DEFAULTS["images"]["skip_cache"],
        )
    ...
    await makeSpritesheets(
        images, instance_id, config["path"], config["spritesheetPath"], spriteSize
    )
...
async def make_features(
    ...
    InstanceManager[instance_id].update({"features": features})
...
async def make_umap(
    ...
    n_neighbors: int = Query(
        DEFAULTS["umap"]["n_neighbors"],
        title="Neighbors",
        description="Number of neighbors",
    ),
    min_distance: float = Query(
        DEFAULTS["umap"]["min_distance"],
        title="Min Distance",
        description="Minimum distance",
    ),
    raster_fairy: bool = Query(
        DEFAULTS["umap"]["raster_fairy"],
        title="Raster Fairy",
        description="Use raster fairy",
    ),
):
    """
    ...
    if (
        instance_id not in InstanceManager
        or "features" not in InstanceManager[instance_id]
    ):
        await make_features(
            instance_id,
            batch_size=DEFAULTS["features"]["batch_size"],
            skip_cache=DEFAULTS["features"]["skip_cache"],
        )
...
async def run(instance_id: str = Query(title="Instance ID", description="Instance ID")):
    ...
    await make_umap(
        instance_id,
        n_neighbors=DEFAULTS["umap"]["n_neighbors"],
        min_distance=DEFAULTS["umap"]["min_distance"],
        raster_fairy=DEFAULTS["umap"]["raster_fairy"],
    )
...
def set_defaults(defaults: dict):
    ...
    for key in defaults:
        ...
        if type(defaults[key]) != type(DEFAULTS[key]):
            return {
                "error": "Type of key {} is not the same as in defaults".format(key)
            }, 400
        for subkey in defaults[key]:
            ...
            if type(defaults[key][subkey]) != type(DEFAULTS[key][subkey]):
                return {
                    "error": "Type of subkey {} is not the same as in defaults".format(
                        subkey
                    )
                }, 400
    DEFAULTS = defaults
        ...
        while True:
            ...
            if resp:
                ...
                await manager.broadcast(
                    {"type": "ping", "data": 1, "instance": instance_id}
                )
